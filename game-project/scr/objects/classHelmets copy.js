...
export class classHelmets { // extends objectsLib {
  ...
  get durabilityMax(){
    return this._Obj.durabilityMax;
  }
  get indestructible(){
...
decreaseDurability(amount = 1) {
  ...
  // применить предмет
  use(){
    console.log("применить предмет:: ТИП: " + this._Obj.type + " ИМЯ: " + this._Obj.name + " classHelmets");
    Hero.property.armorClass.addTempBonus(this.armorClass); // Класс Защиты
    // Прочность
  }

  // отменить предмет
  cancelUse(){
    console.log("отменить предмет:: ТИП: " + this._Obj.type + " ИМЯ: " + this._Obj.name + " classHelmets");
    Hero.property.armorClass.subtractTempBonus(this.armorClass); // Класс Защиты
    // Прочность
  }
